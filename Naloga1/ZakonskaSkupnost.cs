    ...
    internal class ZakonskaSkupnost
    {
        ...
        public static List<ZakonskaSkupnost> UvoziIzCSV(string pot)
        {
            ...
            foreach (var vrstica in vrstice)
            {
                ...
                string imeOtrok = podatki[12];
                string priimekOtrok = podatki[13];
                DateTime datumRojstvaOtrok = DateTime.ParseExact(podatki[14], "dd.MM.yyyy", CultureInfo.InvariantCulture);
                string otrokovaSola = podatki[15];
                string najljubšiPredmetOtrok = podatki[16];
                int starostOtrok = int.Parse(podatki[17]);

                Otrok otrok = new Otrok(imeOtrok, priimekOtrok, datumRojstvaOtrok, otrokovaSola, najljubšiPredmetOtrok, starostOtrok);
